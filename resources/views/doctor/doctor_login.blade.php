...
<main>
    <div class="m-6 mb-12 rounded-xl p-6 shadow-xl sm:p-10">
        ...
        <div class="text">
            ...
            <p>
            <h1 class="text-3xl font-semibold">{{auth()->guard('doctors')->user()->name}}</h1> </p>
        </div>
    </div>
    <div class="m-6 mb-12 rounded-xl p-6 shadow-xl sm:p-10 text">
        <p><a href=""
              class="rounded-md bg-green-600 py-2 px-4 font-semibold text-white shadow-lg transition duration-150 ease-in-out hover:bg-green-700 hover:shadow-xl focus:shadow-xl focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2">Пациенты</a>
            <a href="{{route('doctor.records', ['id' => auth()->guard('doctors')->user()->id])}}"
